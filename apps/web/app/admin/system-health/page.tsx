...
export default function SystemHealthPage() {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      {health && (
        <div className="p-6 rounded-xl border border-border/40 bg-gradient-to-br from-card to-muted/30 shadow-sm">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-xs uppercase tracking-wider font-bold text-emerald-600 dark:text-emerald-400">
                ...
                <div className="text-xs text-muted-foreground mt-0.5">
                  Environment: <strong className="text-foreground">{health.environment}</strong> | Active Micro-Replicas: <strong className="text-foreground">{health.activeInstances}</strong>
                </div>
              </div>
            </div>

            <div className="flex items-center gap-6 border-t md:border-t-0 md:border-l border-border/40 pt-4 md:pt-0 md:pl-6">
              <div>
                <div className="text-xs text-muted-foreground">Average Latency</div>
                <div className="text-2xl font-bold text-foreground">{health.averageLatencyMs} ms</div>
              </div>
              <div>
                <div className="text-xs text-muted-foreground">90-Day SLA Uptime</div>
                <div className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">{health.uptimePct}%</div>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Last Probe Banner */}
      {lastProbeResult && (
        <div className="p-4 rounded-xl bg-primary/10 border border-primary/20 text-primary text-xs flex items-center justify-between shadow-sm">
          <div className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {health?.components.map((comp, idx) => (
          <div key={idx} className="p-5 rounded-xl border border-border/40 bg-card shadow-sm space-y-4 hover:border-primary/40 transition-colors">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h4 className="font-semibold text-sm text-foreground line-clamp-1">{comp.name}</h4>
                  <span className="text-[11px] font-mono text-muted-foreground uppercase">{comp.type}</span>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-2 pt-2 border-t border-border/30 text-xs">
              <div>
                ...
                <div className="font-bold text-foreground mt-0.5">{comp.latencyMs} ms</div>
              </div>
              <div>
                <span className="text-muted-foreground">90d Availability:</span>
                <div className="font-bold text-emerald-600 dark:text-emerald-400 mt-0.5">{comp.uptime90dPct}%</div>
              </div>
            </div>

            {comp.details && (
              <div className="p-2.5 rounded-lg bg-muted/40 text-[11px] font-mono text-muted-foreground space-y-1 border border-border/20">
