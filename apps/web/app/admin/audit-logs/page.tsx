...
export default function AuditLogsPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setExportsList(expRes.data.data || []);
    } catch (e) {
  ...
  const handleExecuteErasure = async (e: React.FormEvent) => {
    ...
    try {
      const res = await complianceApi.executeDataErasure(erasureUserId);
      setErasureResult(res.data.data.message);
      setErasureUserId('');
      await fetchData();
    } catch (e: any) {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 text-sm rounded-lg border border-border bg-card text-foreground focus:outline-none focus:ring-2 focus:ring-primary/20"
              />
            </div>
            <select
              value={filterAction}
                  ...
                  {exportsList.length === 0 ? (
                    ...
                    exportsList.map((exp) => (
                      <tr key={exp.id} className="hover:bg-muted/30 transition-colors">
                        ...
                        <td className="py-3.5 px-4">
                          <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-mono font-medium bg-muted border border-border">
                            {exp.format}
                          </span>
                        </td>
                        <td className="py-3.5 px-4 font-medium text-foreground">
                        ...
                        <td className="py-3.5 px-4 font-mono text-xs text-muted-foreground max-w-xs truncate" title={exp.checksumSha256 || ''}>
                        ...
                        <td className="py-3.5 px-4 text-right">
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="inline-flex items-center gap-1.5 px-3 py-1 text-xs font-medium rounded-md bg-secondary text-secondary-foreground hover:bg-secondary/80 transition-colors"
                          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="rounded-xl border border-border/40 bg-card p-6 shadow-sm space-y-4">
            ...
            <form onSubmit={handleExecuteErasure} className="space-y-4 pt-2">
              ...
              {erasureResult && (
                <div className="p-3 rounded-lg text-xs bg-muted border border-border text-foreground font-medium">
                  {erasureResult}
                </div>
              )}
            </form>
          ...
          <div className="rounded-xl border border-border/40 bg-card p-6 shadow-sm space-y-4">
            ...
            <ul className="text-xs text-muted-foreground space-y-2.5 list-disc pl-4 leading-relaxed">
              <li>
                <strong className="text-foreground">Tax Law Overrides:</strong> Section 148 of Income Tax Act mandates financial transaction preservation for 7 years. Invoice tables retain non-identifiable tax metadata.
              </li>
              <li>
                <strong className="text-foreground">Immediate Revocation:</strong> Active sessions, refresh tokens, and mobile push subscriptions are instantly destroyed.
              </li>
              <li>
                <strong className="text-foreground">Immutable Compliance Log:</strong> A tamper-proof record is permanently generated in the `audit_logs` table.
              </li>
            </ul>
          </div>
        </div>
            ...
            {lastExport ? (
              ...
              <form onSubmit={handleCreateExport} className="space-y-4">
                ...
                <button
                  type="submit"
                  disabled={exporting}
                  className="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-medium rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 transition-colors disabled:opacity-50"
                >
      ...
      {selectedLog && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-card border border-border rounded-xl max-w-xl w-full p-6 shadow-2xl space-y-4">
            ...
            <div className="space-y-3 text-xs">
              ...
              <div className="flex justify-between py-1 border-b border-border/20">
                ...
                <span className="font-medium text-foreground">{selectedLog.userName}</span>
              </div>
              <div className="flex justify-between py-1 border-b border-border/20">
              ...
              <div>
                <span className="text-muted-foreground block mb-1">Payload JSON:</span>
                <pre className="p-3 rounded-lg bg-muted text-[11px] font-mono text-foreground overflow-x-auto max-h-48 border border-border/40">
