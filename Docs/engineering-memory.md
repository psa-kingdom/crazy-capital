...
| ADR-021 | Version-Controlled Prisma Migrations & Safe Deployment Workflow (Baseline `0_init` + `migrate deploy`) | **Approved** | Replaces unsafe `db push --accept-data-loss` with deterministic version-controlled SQL migrations applied via `prisma migrate deploy` during deployment. |
| ADR-022 | Canonical Frontend and Admin Consolidation (Single Next.js App Router in `apps/web/app/admin`) | **Approved** | Retires obsolete standalone `apps/admin` workspace; unifies public marketing, customer portal, partner hub, and full admin cockpit under `@cc/web`. |
| ADR-023 | Strict Origin Validation for Credentialed CORS Requests (`isOriginAllowed` + regex host matching) | **Approved** | Eliminates dynamic wildcard origin reflection; enforces regex matching for `*.crazycapital.in`, localhost dev ports, and configured origins with credentials. |
...
| Aug 2026 | **Sprint 10 / Vertical Slice 2.3 Delivered (Intelligent Task Engine & Workload Balancing)**: Domain 6 Workflow Task Engine & Staff Workload Balancing Architecture. Upgraded Prisma schema with enterprise `Task` model (tracking `priority`, `taskType`, `requiredSkill`, `department`, `estimatedHours`, `slaDueAt`, `slaStatus`, `assignmentReason`, `assignmentScore`, `completionNotes`) and `TaskAssignmentHistory` audit log. Implemented `TasksService` and `TasksController` featuring: automatic task generation on stage entry (`createStageTask`), deterministic intelligent routing algorithm (Skill match $+40$ pts, Department match $+30$ pts, Branch match $+15$ pts, Capacity $+15 \times (1 - \text{utilization})$, Overloaded penalty $-50$ pts, deterministic tie-breaking), candidate recommendation rankings (`/tasks/:id/candidates`), task reassignment with audit logging, task lifecycle execution (`PENDING` $\rightarrow$ `IN_PROGRESS` $\rightarrow$ `COMPLETED`), and team capacity aggregation. Integrated task creation hooks in `ApplicationsService.create` and `ApplicationsService.transitionStage`. Admin Task Engine & Workload Workbench (`apps/admin/app/tasks/page.tsx`) with Operational Task Queue, Staff Workload Balancing Radar, candidate reassignment modal, task completion modal, manual creation flow, and inspect drawer. 140/140 unit tests passing across all 16 test suites, 100% clean Turborepo monorepo build, **218/218 real Google Chrome browser acceptance checks passing (100%)**. | Slice 2.3, ADR-012 | Docs/engineering-memory.md, apps/api/src/modules/tasks, apps/admin/app/tasks, @cc/types, scripts/browser-acceptance-suite.js |
| Aug 2026 | **Sprint 11 / Vertical Slice 2.4 Delivered (Branch Hierarchy & Regional Operations Hubs)**: Domain 2 / Domain 12 Multi-Branch Hierarchy & Regional Rollup Architecture. Created Prisma models `Region` and `BranchTarget` with unique `@@unique([branchId, targetPeriod])`, and extended `Branch` (with `regionId`, `branchManagerId`, `branchType`, `addressLine`, `city`, `state`, `pincode`, `phone`, `email`) and `User` with managed region & branch relations. Implemented `BranchesService` & `BranchesController` featuring: Organization $\rightarrow$ Region $\rightarrow$ Branch multi-tier hierarchy, strict role-based data scoping (`SUPER_ADMIN`/`ADMIN` org-wide, `REGIONAL_MANAGER` regional hub and member branches, `BRANCH_MANAGER` assigned branch), branch target management with authoritative achievement calculation from paid invoices (`Invoice.amount` with status `PAID`) and completed cases (`Application.status == 'COMPLETED'`), attainment percentage & variance calculation, target status determination (`ACHIEVED`, `ON_TRACK`, `AT_RISK`, `MISSED`), and regional rollup performance matrix consolidation (`getPerformanceMatrix`). Created Admin Branch Hierarchy & Regional Hubs Workbench (`apps/admin/app/branches/page.tsx`) with 4 KPI overview cards, Regional Hubs hierarchy view, Branch Directory with search/filters, Target Management table with variance analysis, Regional Performance Scorecards, and interactive modals/drawers (+New Branch, +New Regional Hub, Set Target, Branch 360 Inspector). 149/149 automated unit tests passing across all 17 test suites, 100% clean Turborepo monorepo build, **241/241 real Google Chrome browser acceptance checks passing (100%)**. | Slice 2.4 | Docs/engineering-memory.md, apps/api/src/modules/branches, apps/admin/app/branches, @cc/types, scripts/browser-acceptance-suite.js |
| Aug 2026 | **Sprint 12 / Vertical Slice 2.5 Delivered (RazorpayX Automated Partner Payouts)**: Domain 9 Partner Payouts & Automated Financial Execution Architecture (`ADR-011`, `ADR-014`). Upgraded Prisma schema with `Payout` model fields (`payoutReference`, `idempotencyKey`, `provider`, `providerPayoutId`, `fundAccountId`, `contactId`, `payoutMode`, `accountNumberMasked`, `ifsc`, `failureReason`, `initiatedById`, `initiatedAt`, `paidAt`) and extended `User` model with partner bank coordinates (`bankAccountNumber`, `bankIfsc`, `bankAccountName`, `upiId`). Implemented `PayoutProvider` abstraction with `RazorpayXPayoutProvider` supporting live REST calls (`/v1/payouts`, `/contacts`, `/fund_accounts`) with `X-Payout-Idempotency` and deterministic mock fallback engine with realistic UTR generation for safe testing. Upgraded `PayoutsService` & `PayoutsController` featuring: strict Admin-only authorization, commission status checks (`APPROVED` only), server-side authoritative amount derivation from `Commission.amount`, duplicate disbursement prevention, idempotent financial execution, state machine (`PENDING_PAYOUT` $\rightarrow$ `PROCESSING` $\rightarrow$ `PAID` / `FAILED`), live RazorpayX operational balance check (`GET /payouts/razorpayx/balance`), status sync (`POST /payouts/:id/sync`), and retry engine (`POST /payouts/:id/retry`). Upgraded Admin Commissions & Payouts Workbench (`apps/admin/app/commissions/page.tsx`) with RazorpayX operational float ribbon, 4 KPI overview cards, Commission Approval Queue with `⚡ Execute RazorpayX Payout` modal (showing masked bank coordinates, transfer mode selector, and double-click protection), and RazorpayX Payouts Ledger with UTR numbers and `Inspect 360` audit drawer. 158/158 automated unit tests passing across all 18 test suites, 100% clean Turborepo monorepo build across all 8 packages, **261/261 real Google Chrome browser acceptance checks passing (100%)**. **PHASE 2 COMPLETE**. | Slice 2.5, ADR-011, ADR-014 | Docs/engineering-memory.md, apps/api/src/modules/partners, apps/admin/app/commissions, @cc/types, scripts/browser-acceptance-suite.js |
| Aug 2026 | **Phase 3 Delivered (Ecosystem Scale & Network Effects)**: Implemented Slices 3.1-3.4 (Partner Portal V2, Franchise 70/30 Revenue Split & Monthly Settlements Ledger, Multi-Tier Sub-Partner Referral Tree & Custom Discount Coupons, DigiLocker & Statutory Identity Verification with Pan/GSTIN validations). 173/173 tests passing across 25 suites, 100% clean Turborepo monorepo build. | Slices 3.1-3.4 | Docs/engineering-memory.md, apps/api, apps/web, @cc/types |
| Aug 2026 | **Phase 4 Delivered (Automation, AI & Document Intelligence)**: Implemented Slices 4.1-4.4 across backend, database, and frontends. **4.1 AI-Powered Lead Scoring & Priority Queue**: Multi-factor explainable scoring engine (Source Intent, Service Ticket Size, Data Completeness, Engagement Velocity, Channel Trust), priority sales queue, and real-time score factor breakdown modal. **4.2 Document OCR & Automated Verification Assistant**: `IntelligentOcrProvider` with Levenshtein fuzzy matching on PAN, GSTIN, Aadhaar, Bank Cheque, and side-by-side discrepancy review workbench. **4.3 Crazy Capital AI Operations Copilot**: Interactive multi-turn operations copilot drawer with Indian compliance knowledge base (MCA SPICe+, GST REG-01, NICE class IPR, DPIIT Startup exemptions), gate blocker analysis, and automated WhatsApp/Email/SMS message drafter. **4.4 Predictive Revenue & Turnaround Analytics**: Pipeline-weighted revenue forecasting, stage turnaround velocity modeling, and preventive bottleneck early warning radar with actionable intervention deployment. 190/190 automated unit tests passing across 29 test suites, 100% clean Turborepo build. **PHASE 4 COMPLETE**. | Slices 4.1-4.4, ADR-018 | Docs/engineering-memory.md, apps/api, apps/web, @cc/types, @cc/validation |
